...
extern "C" {
    ...
    void lstm_wx_sequential_infer(
        ...
        for (i = 0; i < time_step; i++) {
           //Wx*x+b; Wx:[3H,D] x:[T, D, N]
           cblas_sgemm(CblasRowMajor, CblasNoTrans, CblasNoTrans, m, n, k, 1, w_x, k,x+i*k*n, n, beta, gemmC1, n);
           //Wh*h+tmp; Wh:[3H,H] h:[H,N]
           cblas_sgemm(CblasRowMajor, CblasNoTrans, CblasNoTrans, m, n, hid, 1, w_h, hid, h_t1, n, beta, gemmC2, n);
           // sigmoid for f,i,o, tanh for c_wave
           #pragma omp parallel for
           for (j = 0; j < hid*batch_size; j++) {
               if (b==NULL){
		           r_t[j] = 1 / (1 + exp(-gemmC1[j]-gemmC2[j]));
		           z_t[j] = 1 / (1 + exp(-gemmC1[j+hid*batch_size]-gemmC2[j+hid*batch_size]));
		           n_t[j] = tanh(gemmC1[j+2*hid*batch_size] + r_t[j]*gemmC2[j+2*hid*batch_size]);
               }
               else{
 		           r_t[j] = 1 / (1 + exp(-gemmC1[j])-gemmC2[j]) + b[j/batch_size];
		           z_t[j] = 1 / (1 + exp(-gemmC1[j+hid*batch_size]-gemmC2[j+hid*batch_size])) + b[j/batch_size + hid];
		           n_t[j] = tanh(gemmC1[j+2*hid*batch_size]) +r_t[j]*gemmC2[j+2*hid*batch_size] + b[j/batch_size + 2*hid];
               }
               h_t[j] = (1-z_t[j]) * n_t[j] + z_t[j] * h_t1[j];
            }
	        if(return_sequences){
                h_t1 = h_t;
