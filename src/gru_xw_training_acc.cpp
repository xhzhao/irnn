...
void gru_xw_single_bwd(int T, int D, int N, int I, int H,
   ...
   /*out*/  float* dwx, //[I,3H]
    ...
    memcpy(dht1, dhy, N * H * sizeof(float));
    #pragma omp parallel for collapse(2)
    for(i=0; i < N; ++i){
        for(j=0; j < H; ++j){
    ...
    if(D==2){
        ...
        memcpy(back_dht1, dhy + N*H, N * H * sizeof(float));
        #pragma omp parallel for collapse(2)
        for(i=0; i < N; ++i){
            for(j=0; j < H; ++j){
    ...
    for(t = T - 1; t >= 0; --t){
        ...
        for( i=0; i < N; ++i){
            for( j=0; j < H; ++j){
                ...
                dar[zid] = dat[zid] = dht1[id] * (ht1[i*D*H+j] - nt[id]) * zt[id] * (1 - zt[id]);
                dar[rid] = dat[rid] = dat[nid] * Mnht[id] * rt[id] * (1 - rt[id]);
                dar[nid] = dat[nid] * rt[id];
    ...
    for(t = 0; t < T; ++t){
        ...
        for( i=0; i < N; ++i){
            for( j=0; j < H; ++j){
                ...
                dar[zid] = dat[zid] = back_dht1[id] * (back_ht1[i*D*H + H + j] - 
                  nt[id]) * zt[id] * (1 - zt[id]);
                dar[rid] = dat[rid] = dat[nid] * back_Mnht[id] * rt[id] * (1 - rt[id]);
                dar[nid] = dat[nid] * rt[id];
        ...
        cblas_sgemm(CblasRowMajor, CblasNoTrans, CblasTrans, N, H, 3 * H, 1, 
          dar, 3 * H, back_wh, 3 * H, 1, back_dht1, H);
        // dwh = ht1.T * da    [H,3H] = [H,N] * [N,3H]
        cblas_sgemm(CblasRowMajor, CblasTrans, CblasNoTrans, H, 3 * H, 
          N, 1, back_ht1+H, D*H, dar, 3 * H, 1, back_dwh, 3 * H);
        // dbx = e * da       [1,3H] = [1,N] * [N,3H]
    if(dbx and dbh){
        #pragma omp parallel for
        for(i = 0; i < 3 * H; ++i){
            for(j = 0; j < N; ++j){
                back_dbx[i] += dat[j * 3 * H + i];
    ...
    memcpy(dhx, dht1, N * H * sizeof(float));
    if(D==2) memcpy(dhx+N*H, back_dht1, N * H * sizeof(float));
}
...
int  gru_xw_forward(int L,
                    ...
                    int mode)
{
    gru_xw_seq_forward(L, T, D, N, I, H, x, hx, wx, wh, bx, bh, y, hy, ws);
}
