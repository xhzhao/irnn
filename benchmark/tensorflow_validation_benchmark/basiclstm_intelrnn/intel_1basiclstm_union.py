...
xinput = np.tile(X_train,seq_length).reshape(seq_length, input_size, batch_size)
#    for i in range(0,seq_length-1):
#        xinput[i]=X_train
    
ytarget = np.random.rand(hidden_size, batch_size).astype(np.float32)
ytarget = np.tile(y_train,hidden_size).reshape(hidden_size, batch_size)
    

with tf.device('/cpu:0'):
    x = tf.placeholder(tf.float32, [seq_length, input_size, batch_size], name="x")
    y = tf.placeholder(tf.float32, [hidden_size, batch_size], name="y")
    
    cell = intelrnn_tensorflow.BasicLSTMCell(hidden_size)
...
#    print ("Compiling...")
    ...
    output, _cell_state = intelrnn_tensorflow.static_rnn(cell, x, dtype=tf.float32)
 
    intel_out = tf.unstack(output,seq_length)
    cost = tf.reduce_sum((intel_out[-1] - y) ** 2)

#    cost = tf.reduce_sum((output[-1] - yi) ** 2)
...
#    print ("--- %s seconds" % (time.time() - start))
    ...
    for i in range(0, n_batch):
        ...
        session.run(output, feed_dict={x: xinput})
    end = time.time()
    print ("Forward:")
    ...
    for i in range(0, n_batch):
        ...
        session.run(train_op, feed_dict={x: xinput, y:ytarget})
    end = time.time()
