    ...
    def c_code(self, node, name, inputs, outputs, sub):
        ...
        ccode = """
            ...
            if (embed_dims != PyArray_DIMS(%(W_x)s)[1]) {
                PyErr_Format(PyExc_RuntimeError, \"GRU: X * W_x size error, %%d, %%d\", embed_dims, PyArray_DIMS(%(W_x)s)[1]);
                goto gru_fail;
            }
        ...
        ccode += """
            ...
            for (int i = 0; i < time_step; i++) {
                ...
                cblas_%(dtype)sgemm(CblasRowMajor, CblasNoTrans, CblasNoTrans, m[0], n[0], k[0],
                                    1.0f, A[2], k[0], B[2], n[0], 0.0, C[2], n[0]);
                // Save hht
                ...
                #pragma omp parallel for
                for (int j = 0; j < mn; j++) {
                    z_ptr[j] = (%(d)s)( ((double)1.0 - (double)(zt_ptr[j])) * (double)(B[0][j]) + (double)(zt_ptr[j]) * (double)(hcan_ptr[j]));
                }
